...
package com.example.yj.mapapp.exception;
...
import com.example.yj.mapapp.util.ConfigUtils;
import com.example.yj.mapapp.util.PackageHelper;
...
public class CrashHandlers implements UncaughtExceptionHandler {
    public static final String TAG = CrashHandlers.class.getSimpleName();
...
//    }
    ...
    private void saveCrashInfoToFile(Throwable ex) {
        ...
        try {
        	...
        	savePath = ConfigUtils.Crash_PATH;
			File file = new File(savePath);
			if (!file.exists()) {
				file.mkdirs();
			}
//            String fileName1 = savePath  + CRASH_REPORTER_EXTENSION;
            String fileName1 = savePath+fileName+".txt";
            // 保存文件
           
            
         // 输出临时下载文件
			File tmpFile = new File(fileName1);
			FileOutputStream trace = new FileOutputStream(fileName1);
            
//			FileOutputStream trace = mContext.openFileOutput(fileName1, Context.MODE_PRIVATE);
